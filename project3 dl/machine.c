...
{
  if ( opcode == NULL || imm_or_addr == NULL || reg1 == NULL || reg2 == NULL ){
    return 0;
  }
...
void print_instruction(Word instruction){ 
  ...
  if(opcode > 0 && opcode < 21){
    ...
    if(opcode >= 1 && opcode <=19){
      printf("\tR%02d", (instruction & 0x3E0) >> 5);
    }
...
int load_program(Word memory[], unsigned int start_addr, const Word program[],
  ...
  if(memory == NULL || program == NULL || 
     program_size + (start_addr/4) > 2048 || start_addr %4 != 0 ){
    return 0;
  }
  for (i = start_addr; i < program_size; i++){
    unsigned opcode_temp;
    unsigned int addr_imm;
    opcode_temp = program[i] & 0xF8000000  >> 27;
    addr_imm = (program[i] & 0x7FFFC00) >> 10;
    if ((opcode_temp >= 16 && opcode_temp <=18) || opcode_temp == 20){
      addr_imm += start_addr;
    }
    if(addr_imm > 8191){
      return 0;
